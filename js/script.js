...
(function () {
	...
	let
			...
			plugins = {
				...
				vide:                    $('.vide'),
				togglePassView:          $('.form-input-wrap'),
				selectFilter:            $("select"),
				rdRange:                 $('.rd-range'),
				slick:                   $('.slick-slider'),
				stepper:                 $("input[type='number']"),
				alertClear:              document.querySelector('.product-filter-badge-clear'),
				counter:                 document.querySelectorAll('.counter:not(.disabled)'),
	...
	$(function () {
			...
			for (let i = 0; i < plugins.countdown.length; i++) {
				let
						...
						countdown = aCountdown({
							...
							tick:   100,
							onTick: function () {
								let counters = node.querySelectorAll('.countdown-counter')
								for (let i = 0; i < counters.length; i++) {
									let counter = counters[i]
									$(counter).html($(counter).text().replace(/./g, '<span>$&</span>'))
								}
							}
						});
			}
		...
		// Toggle Pass View
		if (plugins.togglePassView.length) {
			for (let i = 0; i < plugins.togglePassView.length; i++) {
				let
						node = plugins.togglePassView[i],
		...
		// Alert Clear
		if (plugins.alertClear) {
			let alerts = document.querySelectorAll('.product-filter-badge');
			plugins.alertClear.addEventListener('click', function () {
